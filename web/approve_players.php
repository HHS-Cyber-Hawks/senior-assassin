...
<html>
  ...
  <body>
    ...
    <div class="button-header">
      <div>
        <span>
            <a href="index.php?round= <?php echo $round; ?>"<button class="button">Player List</button>
            <?php if(isAdmin()){ ?>
            <a href="assignment_display.php?round= <?php echo $round; ?>"><button class="button">Assignments</button></a>
            <a href="approve_players.php?round= <?php echo $round; ?>"><button class="current-button">Yet to Pay</button></a>
            <?php } else {  //if(isAdmin()) ?>
...
$sql = <<<SQL
            SELECT id, user_name, email, display_name, has_paid, is_admin
            FROM users
            ORDER BY user_name
SQL;
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        if($row["is_admin"] == 0 && $row["has_paid"] == 0)
        {
          ...
          echo "</td>";
          echo "<td>";
          echo "<button onclick = 'approvePlayer(" . $row["id"] . ")'> Has Paid </button>";
          echo "</td>";
          echo "</tr>";
        }
